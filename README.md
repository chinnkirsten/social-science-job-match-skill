...
按Ponytail的最小实现原则调整：入口按任务路由，详细标准只维护在对应参考文件；现有缓存和生成器直接复用，不新增Agent框架或依赖。搜索先用少量完整JD校正方向，再扩展、去重、核验；不设置会减少目标数量的查询配额。

新增同运行目录失败重试的公开提取缓存。材料、采集时间、画像、指令和模型配置不变且未过期时，复用SourceScout结果；个人匹配、完整改写与Auditor仍重新执行。完整岗位检查点恢复保持原有规则，强制刷新和源内容变化均不能走旧提取。详见[增量更新](references/run-and-evaluate.md)。

离线回归比较失败重试与从头运行的岗位、简历版本和申请动作，要求内容一致；检查过期、变化、损坏和强制刷新。模拟场景可少一次提取请求，但不代表已验证真实求职准确率或固定费用节省。20家公司目标、每岗至少3组对照和2条事实改写、人工复核与Word排版标准不变。
