...
本仓库采用“注册表 + 统一记录 + 审计闸”，不把所有上游源码复制到 Skill 中。复制源码会造成许可证混用、版本失控、重型依赖和安全更新滞后；注册表用固定版本记录可复现依据，适配器只需输出统一字段。
...
```json
{
  ...
  "candidate_evidence": [
    {"id":"E1","source_id":"resume","locator":"第1页项目经历第1条","text":"原始经历事实","state":"completed","confirmed":true}
  ],
...
示例只说明字段，不是可投数据。运行`build_evidence_basis.py`后得到`evidence_basis`和`corpus_summary`。岗位报告JSON同时嵌入完整`corpus`，校验器从中重算统计。空JD、过期采集、超出城市范围、重复来源以及断裂的证据引用会报错。详细约定见[validation.md](../references/validation.md)。

## 推荐流水线

1. **本地解析**：PDF/DOCX优先用Docling或现有文档工具；保留页码、段落位置和原文件，不上传私人材料。
2. **岗位发现**：先查本地库、官网、官方ATS；Crawl4AI只抽取允许访问的公开页。JobSpy只补充境外线索。
3. **语料归一化**：保留中文JD原词；O*NET/ESCO添加标准引用，不覆盖原词。Tabiya或ESCO Skill Extractor的结果标为模型候选。
4. **证据补录**：借鉴Compass逐项向用户确认经历；未确认的技能留在待确认项。
5. **差距与改写**：借鉴Resume Matcher呈现关键词和差距；每条改写必须关联候选人证据编号。
6. **离线评估**：需要比较分类器时用MELO或TGRE；评估集与中国岗位不一致时明确限制。
7. **审计交付**：运行语料汇总、集成注册表检查和`validate_jobs.py`，再逐页检查Word。
